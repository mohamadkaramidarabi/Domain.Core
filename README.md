...
```bash
dotnet add package Selma.Domain.Core
```
...
cd Domain.Core
dotnet test tests/Domain.Core.Test/Domain.Core.Test.csproj
```

Build a NuGet package locally:

```bash
dotnet pack src/Domain.Core/Domain.Core.csproj -c Release -o ./artifacts
```

## Releasing

Releases are published to [nuget.org](https://www.nuget.org/packages/Selma.Domain.Core) via GitHub Actions when a version tag is pushed (e.g. `v1.0.0`). The tag version should match the `<Version>` in `Domain.Core.csproj`.
