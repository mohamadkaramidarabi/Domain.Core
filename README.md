...
```bash
dotnet add package Domain.Core
```
...
Releases are published to [nuget.org](https://www.nuget.org/packages/Domain.Core) via GitHub Actions when a version tag is pushed (e.g. `v1.0.0`). The tag version should match the `<Version>` in `Domain.Core.csproj`.
